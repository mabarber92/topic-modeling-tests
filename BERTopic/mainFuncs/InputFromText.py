...
def InputFromText(textPath, outPath = None, seqCap = 512, minSplit = 2, adaptiveSplit = False, uriCol = False, shingle=False):
    ...
    outDictList = []
    for idx, ms in enumerate(msDictList):
        ...
        # If the length of the MS is only one token - it is not useful for topic modelling and skipped        
        if tokCount == 1:
            finalIt=True
        
        
        # If there are individual words that are very long - there is too much OCR noise, skip the milestone. As this messes with attempts to create meaningful token-wise splits
        # and messes with embedding exercise
        
        if len(tokens) > 0:
            largestTok = len(max(tokens, key=len))        
            if largestTok > 20:
                finalIt = True
        ...
        while remainingToks > 0 and not finalIt:            
    ...
    print("Input csv created for " + str(textPath))
    print("Csv length:" + str(len(outDf)))
    if outPath is not None:
        outDf.to_csv(outPath, index=False, encoding='utf-8-sig')
    return outDf                
        
   
# path = "D:/OpenITI Corpus/corpus_2022_1_6/data/0550Wisyani/0550Wisyani.Siyar/0550Wisyani.Siyar.Kraken2107192213161-ara1"
# outPath = "C:/Users/mathe/Documents/Github-repos/topic-modeling-tests/memory-test.csv"
...
# out = InputFromText(path, outPath, seqCap=512, adaptiveSplit=True, shingle=True)
